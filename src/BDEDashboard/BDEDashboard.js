  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/v1/editjob`,
        updatedJob
      );
        ...
        {state.isModalOpen && state.selectedJob && (
          <div className="job-modal-overlay">
            <div className="job-modal-bde">
              ...
              <div className='btns'>
                         <Link to={`/bdestudentsappliedjoblist/${state.selectedJob.job_id}`} className='applied-students-list'>Applied Students</Link>
                         <button className="edit-btn" onClick={() => handleEditClick(state.selectedJob)}>Edit</button>
                     </div>
            </div>
          </div>
        )}
